...
> アプリは公証を受けていません。右クリック → 開く の手順は一度だけ必要な macOS の仕様であり、voiceBox 自体の問題ではありません。
...
バイナリリリースは個人利用に限り無償で提供しています。ソースコードは公開していません。voiceBox はオープンソースのモデルやフレームワーク（下記クレジット参照）を基盤としています。
